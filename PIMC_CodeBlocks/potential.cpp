...
*  Constructor.
*
*  We create a Lennard-Jones hexagonal pore, which will eventually use a lookup table
...
double LJHexPorePotential::valueV(const dVec &rhat)
{
    ...
    y = rhat[0]*sin(rhat[1]);
    /*cout << "********** New Position ********** \n";
    cout << "The sidelength is " << t << " and the point is: ";
    cout << "(" << x << ", " << y << ")\n";
    cout << "The conditions are: \n";
    cout << -t/2.0 << " <= " << x << " <= " << t/2.0 << "\n";
    cout << -t << " < " << x << " < " << -t/2.0 << "\n";
    cout << t/2.0 << " < " << x << " < " << t << "\n";*/
...
void LJHexPorePotential::initLookupTable(const double _dr, const double _dtheta, const double R, const double theta) {
    ...
    //cout << "Made it to initLookupTable" << endl;
	dr = _dr;
...
//    thetaLength = int(theta/dtheta);
    ...
    //cout << "Made it through initLookupTable" << endl;
}
...
double LJHexPorePotential::V(const dVec &r)
{
    ...
    int section, rindex;
    cout << "Calling Hex Pore V function" << endl;
    x = r[0];
    y = r[1];
    theta = atan2(y, x);
    ...
    if (rindex >= rLength-1){
        cout << "R index too large, returning extV[1]" << endl;
        return extV[1];
    }
    else{
//      cout << bilinearInterp(twoDlookupV, extV, rval, theta) << endl;
        cout << "Entering bilinear interp" << endl;
...
double LJHexPorePotential::bilinearInterp(const Array<double,2> &VTable,
    ...
    //    cout << "Made it to bilinear interp" << endl;
    ...
    thetaindex2 = thetaindex1 + 1;
        ...
        cout << "rindex1 = " << rindex1 << endl;
        cout << "rindex2 = " << rindex2 << endl;
        cout << "thetaindex1 = " << thetaindex1 << endl;
        cout << "thetaindex2 = " << thetaindex2 << endl;
        cout << "thetaLength = " << thetaLength << endl;
    ...
    // Get the lookup table values
    fr1theta1 = VTable(thetaindex1, rindex1);
    fr1theta2 = VTable(thetaindex2, rindex1);
    fr2theta1 = VTable(thetaindex1, rindex2);
    fr2theta2 = VTable(thetaindex2, rindex2);
    ...
    cout << "Interp Val is: " << interpval << endl;
    return interpval;
}
