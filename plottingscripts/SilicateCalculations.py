...
def FittingAllSpaceCalculation(f,epsilon):
...
def piOverThreePlots(t,T,radius, epsilon, sigma, density):
    ...
    plt.title(r"$\frac{\pi}{3}$ Plots, Side Length = %2.1f" % (t),y=1.01)
    plt.ylim(-4,1)
    plt.show()
...
def MinimaSearch(T, t, radius, epsilon, sigma, density):
    ...
    plt.title('Scaling of Minima Difference with Array Size, Side Length = %d' % (t))
    plt.show()
...
def EpsilonFit(t, T, radius, epsilon, sigma, density):
    ...
    fvec = np.linspace(t-1.25, t-.9, 500)
    silicate = np.array([])
    for j in range(len(fvec)):
    ...
    plt.legend()
    plt.title('Minima of AllSpace and Silicate Calculations')
    plt.show()
    ...
    plt.plot(fvec,fittedVals, label='Fitted All Space Calc, Epsilon = %f' %(popt[0]))
    plt.legend()
    plt.ylim(-4,1)
