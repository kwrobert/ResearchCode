...
//   18 Jul 1996     V0.2 (allow negative integrals)
...
//////////////////////////////////////////////////////
void
PlaneAdaptive::Rule(Integrand& F,Plane& R,real& TheResult,
  real& TheError,real& HalfValueRadius)
  {
  int i,j,p,Type,nr,number;
  real Null[6],OrbitContrib[7],below,above,fraction,TheAbsResult;
  real Tres = 50*REAL_EPSILON;
  real noise,r,r1,r2,r3,Deg5,Deg3,Deg7,Deg1,sumval,Fx,sumabsval;
  real A=1.0/R.ScaleX() , B=1.0/R.ScaleY();
  Point x[8];

  TheResult = 0.0;  TheAbsResult = 0.0;
  ...
  for (Type=0;Type <=3;Type++){
     for (nr=0;nr<K[Type];nr++){
	    ...
	    sumabsval = fabs(Fx);
            for (j=1 ; j<number ; j++)
            { Fx = F(x[j]); 
              sumval += Fx; sumabsval += fabs(Fx); 
            }
            TheAbsResult += (OrbitContrib[p] = Weight[0][p]*sumabsval);            
  ...
  do{
    ...
    } while (fabs(above) < fabs(TheAbsResult)*0.5);
    // 50% circle is lying between orbits d[i-1] and d[i]
    fraction = (TheAbsResult*0.5-below)/OrbitContrib[d[i]];
    if (d[i] < K[1]+K[2])
       {
...
//std::cout<<"Bijdrage per orbiet (toenemende afstand):"<<endl;
//for(p=0; p<=6 ; p++)
//{std::cout<<d[i]<<"  "<<OrbitContrib[d[p]]<<" "<<RingRadius[d[p]]<<endl;}
//std::cout<<"HalfValueRadius = "<<HalfValueRadius<<"  past orbit "<<i<<endl;
//std::cout << "Benadering voor vlak= " <<TheResult<<endl;
//std::cout << "Nulregels:" <<endl;
//for (i=0 ; i<=5 ; i++){std::cout<<Null[i]<<endl;}
//std::cout <<"---------------------------"<<endl;
  //    Compute errors.
