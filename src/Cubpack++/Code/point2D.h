...
//    1 May 1999     V1.2 ( addition norm operator   )
// 
/////////////////////////////////////////////////////////
...
norm(Point_2D& p,int type) 
  ...
  case 0:		// 00-norm  
    { 
      nrm = max(fabs(p[0]),fabs(p[1])); 
      break; 
    } 
  case 1: 		// 1-norm 
    { 
      nrm = (fabs(p[0])+fabs(p[1])); 
      break; 
    } 
  case 2: 		// 2-norm   
    { 
      nrm = (sqrt(p[0]*p[0]+p[1]*p[1])); 
      break; 
    }  
  default: 
    {
      Error(True,"This norm is not implemented.");
    }
  } 	// end switch 
  return nrm; 
  } 
